...
function generatePassword() {
  ...
  var characterLength = prompt(
    "How long would you like your password to be? (Between 8 and 128)"
  );
  ...
  var confirmNumbers = confirm(
    "Would you like your password to have NUMBERS in it?"
  );
  var confirmSpecial = confirm(
    "Would you like your password to have SPECIAL characters in it?"
  );
  ...
  for (var index = 0; index < characterLength; index++) {
    ...
    userPassword += userCharacters.charAt(
      Math.floor(Math.random() * userCharacters.length)
    );
  }
  return userPassword;
}
